    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage('دیدگاه جدید',
                                 'دیدگاه جدیدی برای «{}» دریافت شد لطفا برای مشاهده آن به لینک زیر مراجعه فرمایید:\n {}{}'.format(post, current_site, reverse(
                                     'blog:detail', kwargs={'slug': post.slug, 'id': post.id}
                                 )),
                                 to=[author_email])
            email.send()
        if user_email:
            email = EmailMessage('ثبت دیدگاه',
                                 'دیدگاه شما ثبت شد.به زودی پاسخ آن برایتان ارسال میگردد',
                                 to=[user_email])
            email.send()
        if parent_email:
            email = EmailMessage('پاسخ برای نظرتان',
                                 'پاسخی برای «{}» دریافت شد لطفا برای مشاهده آن به لینک زیر مراجعه فرمایید:\n {}{}'.format(post, current_site, reverse(
                                     'blog:detail', kwargs={'slug': post.slug, 'id': post.id}
                                 )),
                                 to=[parent_email])
            email.send()
